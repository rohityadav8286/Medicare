    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/appointments`, {
        ...
        headers: {
          ...
          // Clerk/auth middleware must return JSON for an API request rather
          // than redirecting to an HTML page that fetch treats as HTTP 200.
      ...
      // Read the response once so a malformed response can be reported with
      // useful diagnostics instead of failing later while reading checkoutUrl.
      ...
      try {
        ...
        // Logged below with empty-response cases too.
      }
      if (!res.ok) {
        const message =
      ...
      // A successful booking response must be JSON. Avoid dereferencing a
      // missing response body, which previously caused `checkoutUrl` errors.
      if (!body || typeof body !== "object") {
        console.error("Invalid appointment API response:", responseInfo);
        const message = res.redirected
          ? "Booking was redirected by the server. Please sign out, sign in again, and retry."
          : `Booking server returned an invalid response (HTTP ${res.status}).`;
        toast.error(message, {
          position: "top-center",
        });
        return;
