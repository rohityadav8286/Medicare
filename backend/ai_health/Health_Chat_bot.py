...
def getDescription():
    with open(BASE_DIR / 'MasterData' / 'symptom_Description.csv') as csv_file:
        for row in csv.reader(csv_file):
            description_list[row[0]] = row[1]

def getSeverityDict():
    with open(BASE_DIR / 'MasterData' / 'symptom_severity.csv') as csv_file:
        for row in csv.reader(csv_file):
            try:
...
def getprecautionDict():
    with open(BASE_DIR / 'MasterData' / 'symptom_precaution.csv') as csv_file:
        for row in csv.reader(csv_file):
...
def predict_disease(symptoms_list):
    ...
    pred_proba = model.predict_proba(pd.DataFrame([input_vector], columns=cols))[0]
